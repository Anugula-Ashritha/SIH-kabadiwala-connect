#!/usr/bin/env python3
"""
Kabadiwala Connect - E-Waste Inference Predictor Script
Takes an image path as input, performs inference using the trained
MobileNetV3Small model, and prints:
...
def parse_args():
    parser = argparse.ArgumentParser(
        description="Run E-Waste Image Inference (Battery, Mobile, PCB)"
    )
    parser.add_argument(
        "--image",
        "-i",
    ...
    parser.add_argument(
        ...
        dest="model_path",
        type=str,
        default="./models/ewaste_classifier.keras",
        help="Path to the trained .keras model (default: ./models/ewaste_classifier.keras)",
    )
    parser.add_argument(
        "--classes",
        "-c",
...
def main():
    ...
    if not img_path.exists():
        ...
        print('Tip: Double-check your Windows path and use quotes, e.g.:')
